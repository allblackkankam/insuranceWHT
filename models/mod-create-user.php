...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    $u_mail =mysqli_real_escape_string($conn,test_input($_POST['u_mail']));
	    ...
	    if($send==1){
            $query = "INSERT INTO administrator (secret_key,verifyid,centername,contact,email,pkg,reg_date)
			VALUES('$secretKey','$centerid','$centername','$contact','$email','$pkg',NOW());";
            $query .= "INSERT INTO admin (u_name,u_mail,u_pass,u_com,u_num,session_id,datecreated)
            VALUES('$user_name','$user_mail','$password','$u_com','$u_num',$session_id, now()) ";
